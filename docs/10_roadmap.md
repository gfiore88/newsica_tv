...
- [x] **History Persistente Anti-Ripetizione Musica (ADR 0043)**: la regia salva gli ultimi brani riprodotti, evita di sporcare la history durante i tentativi dry-run del Director, filtra il pool candidato prima della scelta della sorgente e rende ispezionabile in Dashboard la finestra recente con i candidati esclusi.
...
- [x] **Richieste Musicali Freeform da Chat (ADR 0048)**: La richiesta utente entra come vincolo primario nel prompt ACE-Step, con supporto a lingue o dialetti non canonici quando esplicitati.
...
- [x] **Shorts Automatici**: Estrazione e montaggio autonomo dei momenti/news salienti in file `.mp4` verticali pronti per l'upload quotidiano su Shorts. La Dashboard espone anche caption pronta, 5 hashtag pertinenti copiabili, pubblicazione one-click su YouTube/Instagram/TikTok o "tutti i social" e stato persistito nel DB dei post già inviati con timestamp per piattaforma. (Implementato in `shorts_agent.py` e integrato nella Dashboard)
- [x] **Piano Shorts Giornaliero Autonomo con Regia (Director-led)**: il Director genera ogni giorno un piano shorts eterogeneo con vincoli editoriali (news/funfact/tech sempre, meteo/sport/wellness condizionali per rilevanza), il PreparationAgent produce gli asset e Buffer li schedula automaticamente nelle ore calde per piattaforma tramite `dueAt`. Pianificazione automatica agganciata all'alba (`SHORTS_DAILY_DAWN_TIME`) con fallback al primo avvio utile della giornata se la diretta era spenta durante il passaggio all'alba. Aggiunti short **breaking straordinari** durante la giornata (soglia gravità + cooldown) e generazione **just-in-time** vicino all'orario di uscita per mantenere le news più fresche.
- [x] **Refactor Struttura Shorts Plan Execution**: unificata la logica di esecuzione item pianificati in un service applicativo condiviso (`newsica.shorts.plan_executor`) usato sia da Dashboard che da PreparationAgent, riducendo duplicazioni e drift comportamentale.
