...
6. **Validazione Post-Restart (Self-Annealing)**: Ogni volta che applichi una modifica o un refactoring al codice, DEVI lanciare `./manage.sh restart` e analizzare attivamente i log (`tmp/director.log` e `tmp/stream.log`) per confermare che le funzionalità core (regia, FFmpeg, palinsesto) non siano andate in crash. I test funzionali positivi sono vincolanti per chiudere il task.
7. **Unit Test Obbligatori**: Ogni patch deve includere test automatici nuovi o aggiornati sul ramo toccato. Non delegare implicitamente i test a un secondo momento. Se modifichi planner, scheduler, director, parser, repository o API, devi scrivere o aggiornare la suite di unit/regression test nella stessa lavorazione.
