...
def run_loop():
    ...
    while True:
        try:
            ...
            result = process_one_planned_short_item(due_within_minutes=shorts_generation_lead_minutes)
            if result.get("status") not in {"idle", "success", "partial"}:
                print(f"⚠️ [PreparationAgent] Shorts plan item fallito: {result.get('message', 'errore sconosciuto')}")
