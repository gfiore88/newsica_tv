...
<body class="p-4 md:p-8">
    <div class="max-w-7xl mx-auto">
        ...
        <!-- Sezione Audit Log AI -->
        <div class="glass rounded-xl p-6 shadow-xl mb-8">
            <div class="flex items-center justify-between mb-2 cursor-pointer" onclick="toggleAuditLog()">
                <h2 class="text-xs uppercase tracking-widest text-purple-400 font-semibold flex items-center gap-2">
                    🕵️‍♂️ Audit Log AI (Scelte e Ragionamenti)
                </h2>
                <span id="audit-log-toggle" class="text-slate-400 transition-transform duration-200">▼</span>
            </div>
            <div id="audit-log-container" class="hidden mt-4">
                <div id="ai-audit-log" class="font-mono text-[11px] text-slate-300 h-64 overflow-y-auto bg-slate-900/80 p-4 rounded border border-purple-500/30 whitespace-pre-wrap leading-relaxed shadow-inner">
                    In attesa di dati...
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        async function fetchAuditLog() {
            const container = document.getElementById('audit-log-container');
            if (container.classList.contains('hidden')) return; // Non aggiornare se chiuso

            try {
                const res = await fetch('/api/audit-log');
                if (res.ok) {
                    const data = await res.json();
                    const logPanel = document.getElementById('ai-audit-log');
                    ...
                    if (data.lines && data.lines.length > 0) {
                        ...
                        html = html.replace(/\\[(INFO|MUSIC|SCHEDULING|BREAKING|RESTORE|PLAYOUT|WARNING|ERROR)\\]/g, match => {
                            ...
                            if (match.includes('BREAKING')) color = 'text-red-500 font-bold animate-pulse';
                            if (match.includes('RESTORE')) color = 'text-amber-400';
                            if (match.includes('PLAYOUT')) color = 'text-cyan-400';
                            if (match.includes('WARNING')) color = 'text-yellow-400';
                            if (match.includes('ERROR')) color = 'text-red-500 font-bold';
                            return `<span class="${color}">${match}</span>`;
                        });
                        logPanel.innerHTML = html;
                    } else {
                        logPanel.innerHTML = "Nessuna azione recente registrata.";
                    }
                }
            } catch (e) {
                console.error("Errore fetch audit log:", e);
            }
        }
